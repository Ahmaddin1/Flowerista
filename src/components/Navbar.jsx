      ...
      <div className="pointer-events-none sticky top-4 z-50 hidden w-full justify-center md:flex">
        <div className="pointer-events-auto relative flex w-[90%] items-center rounded-full border border-card-border bg-bg/70 backdrop-blur-md px-5 py-3 shadow-[var(--shadow-card)]">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`relative flex items-center gap-1.5 rounded-full px-4.5 py-2.5 text-[11px] uppercase tracking-[2px] transition-colors duration-200 ${active ? "text-text-on-accent" : "text-text hover:text-accent-strong"}`}
                >
                  {active ? (
                    <motion.span
          ...
          <Link
            href="/cart"
            aria-label="Open cart"
            className="relative ml-auto flex h-10 w-10 items-center justify-center rounded-full bg-accent text-text-on-accent"
          >
            <ShoppingCart className="h-5 w-5" strokeWidth={2} />
            {cartCount > 0 ? (
              <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-accent-strong text-[9px] font-bold text-text-on-accent">
                {cartCount}
              </span>
            ) : null}
          </Link>
